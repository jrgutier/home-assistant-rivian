...
request headers AND the request body attached (``rivian.py:986`` and
``rivian.py:988``). None of these classes defines ``__init__``, so Python renders
...
* **Shape-agnostic.** Redaction walks every argument by key name at any depth,
  rather than keying on argument position. ``rivian.py:986`` passes four
  positional arguments while ``rivian.py:988`` passes five, with ``headers`` and
