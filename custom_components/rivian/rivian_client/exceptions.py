...
request headers AND the request body attached (``rivian.py:987`` and
``rivian.py:989``). None of these classes defines ``__init__``, so Python renders
...
* **Shape-agnostic.** Redaction walks every argument by key name at any depth,
  rather than keying on argument position. ``rivian.py:987`` passes four
  positional arguments while ``rivian.py:989`` passes five, with ``headers`` and
